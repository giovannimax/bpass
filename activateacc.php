    ...
    <div class="container">
      <?php
        ...
        if(isset($_POST['Submit'])){
          ...
          if($res > 0){
            $activate = mysqli_query($con, "UPDATE Accounts SET isActive = '1' WHERE RegID = '$RegID'")or die(mysqli_errno());

            $Msg = "ACTIVATION SUCCESS: Admin ID: " . $ID . " activated User ID: " . $RegID . " at " . $now;
            $log = mysqli_query($con, "INSERT INTO Archive_UserActive VALUES('', '$ID', '$Msg', '$RegID', '$now')")or die(mysqli_errno());

        		echo "<script>alert('Account Activated!');window.location.href='admin_view_user_details.php?RegID=$RegID';</script>";
          }else{
            echo "<script>alert('Activation Failed!');window.location.href='admin_view_user_details.php?RegID=$RegID';</script>";
          }
        }
      ?>
    </div>
